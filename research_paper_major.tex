% =============================================================================
% B.Tech Project (Major) -- Research Paper
...
% ---------- Custom Title Page (matches minor evaluation report) ------------
\begin{titlepage}
\centering
...
{\Large End-Term Evaluation Report}\\[1.5cm]
...
This paper proposes an integrated two-layer architecture. The \textbf{caching layer} consists of (1)~a \textbf{thread-level cache} that eliminates redundant data fetches within a single request's execution context and (2)~a \textbf{server-local cache} that absorbs cross-request lookups for immutable, configuration-class data. The \textbf{contention layer} layers five complementary safety mechanisms over the cache to handle single-resource booking correctly: \emph{distributed locking with fencing tokens}, \emph{atomic decrement via Redis Lua scripts}, \emph{optimistic concurrency control with versioned cache entries}, \emph{CRDT-based PN-Counters for eventually-consistent reservations}, and \emph{single-flight request coalescing}. We define formal cache-safety boundaries that govern which data classes are eligible for caching, and a decision matrix that selects the right contention primitive per data class. Evaluation on a production payments platform handling roughly 68{,}000 cross-node operations per second shows a \textbf{4.1\% reduction in cross-node network calls at peak}, an \textbf{11.2\% reduction in p95 latency} and a \textbf{13.0\% reduction in p99 latency}, with \textbf{zero double-allocation incidents} after the contention layer was rolled out. The combined approach is lightweight, requires no new infrastructure beyond a Redis cluster already present for caching, and is portable across language runtimes.
\end{abstract}
...
\begin{tikzpicture}[
    ...
    box/.style={rectangle, draw, rounded corners, minimum height=0.9cm, minimum width=2.7cm, align=center, font=\small, fill=blue!5},
    decision/.style={diamond, draw, aspect=2.2, inner sep=1pt, align=center, font=\scriptsize, fill=yellow!10},
    primitive/.style={rectangle, draw, minimum height=0.9cm, minimum width=2.6cm, align=center, font=\scriptsize, fill=green!8},
    arrow/.style={-Stealth, thick}
]
    \node[box] (req) {Incoming\\Request};
    \node[box, right=of req] (t1) {Tier-1\\Thread Cache};
    \node[decision, right=of t1] (op) {Read or\\authoritative\\write?};

    \node[box, above right=0.3cm and 1.2cm of op] (sf) {Single-Flight\\Coalescer};
    \node[box, right=of sf] (t2) {Tier-2\\Server Cache};
    \node[box, right=of t2] (redis) {Redis /\\Backend};

    \node[primitive, below right=0.3cm and 1.2cm of op] (lua) {Lua atomic\\decrement};
    \node[primitive, right=of lua] (lock) {Lock + fence\\token};
    \node[primitive, right=of lock] (occ) {OCC\\(versioned)};

    \draw[arrow] (req) -- (t1);
    \draw[arrow] (t1) -- (op);
    \draw[arrow] (op.north) -- node[above left, font=\scriptsize]{read} (sf.west);
    \draw[arrow] (op.south) -- node[below left, font=\scriptsize]{write} (lua.west);
    \draw[arrow] (sf) -- (t2);
    \draw[arrow] (t2) -- (redis);
    \draw[arrow] (lua) -- (redis.south);
    \draw[arrow] (lock) -- (redis.south);
    \draw[arrow] (occ) -- (redis.south);
    \draw[arrow, dashed] (redis) to[bend left=35] node[above, font=\scriptsize]{invalidate (Pub/Sub)} (t2);
\end{tikzpicture}
\caption{Integrated two-layer architecture. Reads traverse Tier-1, single-flight coalescer, Tier-2, and only on Tier-2 miss reach Redis. Authoritative writes bypass the cache and select an appropriate contention primitive (Lua, lock+fence, or OCC). Successful writes publish invalidation events that propagate back to Tier-2.}
...
2026. \url{https://oneuptime.com/blog/post/2026-01-21-redis-real-time-inventory/view}

\bibitem{oneuptime2026stampede}
OneUptime Engineering,
``How to Build Cache Stampede Prevention,''
2026. \url{https://oneuptime.com/blog/post/2026-01-30-cache-stampede-prevention/view}

\bibitem{oneuptime2026coalescing}
OneUptime Engineering,
``How to Implement Request Coalescing,''
2026. \url{https://oneuptime.com/blog/post/2026-01-25-request-coalescing/view}
...
INRIA Technical Report, 2011 (foundational reference).

\bibitem{duncan2025crdt}
Duncan, I.,
``The CRDT Dictionary: A Field Guide to Conflict-Free Replicated Data Types,''
2025. \url{https://www.iankduncan.com/engineering/2025-11-27-crdt-dictionary/}
...
\url{https://martin.kleppmann.com/2016/02/08/how-to-do-distributed-locking.html}, 2016 (foundational reference for fence tokens).
